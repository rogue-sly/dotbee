    ...
    match context.config.get_profile(&active_profile) {
        ...
        Err(_) => {
            message::error(&format!(
                "Status: Profile '{}' not found in config!",
                active_profile.red()
            ));
            check_ghost_links(&config_links, context)?;
            bail!(
...
fn check_ghost_links(
    config_links: &IndexMap<String, Link>,
    ...
    if !ghosts.is_empty() {
        println!(
            "{}",
            "Ghost Links (in state but not in current config):"
                .bold()
                .yellow()
        );
        for ghost in ghosts {
        ...
        println!(
            "{}",
            "\nRun 'dotbee sync' to clean up ghost links."
                .italic()
                .dimmed()
        );
    }
